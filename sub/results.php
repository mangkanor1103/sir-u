...
foreach ($positionsData as $position => &$data) {
    ...
    // Special case: if only one candidate in position, they need 50% + 1 votes to win
    if ($candidate_count == 1) {
        $required_votes = floor($total_voters / 2) + 1; // 50% + 1
        if ($candidates[0]['total_votes'] >= $required_votes) {
            $candidates[0]['is_winner'] = true;
            $data['winner_count'] = 1;
        }
    }
    // Normal case: select winners based on max_vote
    else {
        // Check for ties at the border
        $rank = 0;
        $previous_votes = -1;
        $tied_candidates = [];
        $winners_selected = 0;

        // First pass: Mark clear winners and identify ties
        foreach ($candidates as $index => &$candidate) {
            if ($candidate['total_votes'] > 0) {
                // If votes changed, we're at a new rank
                if ($candidate['total_votes'] != $previous_votes) {
                    $rank = $winners_selected + 1;
                    $previous_votes = $candidate['total_votes'];
                    $tied_candidates = [];
                }

                // Store candidates with same vote count
                $tied_candidates[] = &$candidate;

                // If current rank is within max_vote and no ties at boundary, mark as winner
                if ($rank <= $max_vote) {
                    // Only mark winners if there's no tie at the boundary
                    if ($rank < $max_vote || count($tied_candidates) + $winners_selected <= $max_vote) {
                        $candidate['is_winner'] = true;
                        $winners_selected++;
                    } else {
                        // We have a tie at the boundary, mark all tied candidates as tied
                        foreach ($tied_candidates as &$tied_candidate) {
                            $tied_candidate['is_tie'] = true;
                            $tied_candidate['is_winner'] = false;
                        }
                    }
                }
            }
        ...
        // Update winner count
        $data['winner_count'] = $winners_selected;
    }
    ...
    <div class="bg-green-700 text-white py-4 px-6 flex items-center justify-between no-print">
        ...
        <div class="flex space-x-4">
            ...
            <button onclick="window.location.href='../index.php'"
                class="bg-green-800 hover:bg-green-900 text-white px-4 py-1 rounded-md">
    ...
    <div class="container mx-auto px-4 py-6" id="printable-content">
            ...
            <div class="flex justify-center items-center mt-4 text-sm">
                <div class="bg-gray-100 rounded-lg px-4 py-2 inline-flex items-center">
                    <div class="mr-6">
                        ...
                        <span
                            class="text-gray-600 ml-1">(<?php echo $votes_cast; ?>/<?php echo $total_voters; ?>)</span>
                    </div>
                    <div>
                        ...
                        <tbody>
                            <?php foreach ($data['candidates'] as $index => $candidate): ?>
                                ...
                                <tr class="<?php echo $index % 2 === 0 ? 'bg-gray-50' : 'bg-white'; ?> border-b">
                                    ...
                                    <td class="py-2 px-4 font-medium"><?php echo htmlspecialchars($candidate['candidate']); ?>
                                    </td>
                                    <td class="py-2 px-4">
                                        <?php echo !empty($candidate['partylists']) ? htmlspecialchars($candidate['partylists']) : '-'; ?>
                                    </td>
        ...
        <div class="certification mt-10 border-t-2 border-green-700 pt-6">
            <div class="text-center">
                <p class="text-gray-600 mb-8">I hereby certify that the election results shown above are true and
                    accurate.</p>

                <div class="inline-block">
                    <div class="border-b border-gray-400 w-48 h-8 mx-auto mb-1"></div>
                    <p class="font-semibold">Election Administrator</p>
                    <p class="text-sm text-gray-500"><?php echo $_SESSION['admin_name'] ?? 'System Administrator'; ?>
                    </p>
                </div>
